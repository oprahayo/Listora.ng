<div class="grid gap-5">
    ...
    <fieldset>
        ...
        <div class="grid gap-2.5">@foreach(['water' => 'Running water', 'security' => 'Security', 'parking' => 'Parking', 'power' => 'Backup power', 'road' => 'Good access road'] as $key => $label)<label class="flex min-h-10 cursor-pointer items-center gap-2 text-sm text-[#475467]"><input type="checkbox" name="amenities[]" value="{{ $key }}" @checked(in_array($key, $filters['amenities'] ?? [])) class="size-5 rounded border-[#98A2B3] text-[#155EEF] focus:ring-[#155EEF]">{{ $label }}</label>@endforeach</div>
    </fieldset>
</div>
