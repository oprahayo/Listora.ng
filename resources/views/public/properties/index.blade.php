<x-layouts.public>
        ...
        <div class="mx-auto max-w-[1180px] px-4 py-4 sm:px-6 lg:px-0">
            ...
            <div class="mt-4 flex gap-2 overflow-x-auto pb-1 md:hidden" aria-label="Property types">
                @foreach(['' => 'All', 'apartment' => 'Apartments', 'self-contain' => 'Self Contain', 'duplex' => 'Duplexes', 'shared-flat' => 'Shared Flats', 'shop' => 'Shops', 'office' => 'Offices'] as $type => $label)
                    <a href="{{ route('properties.index', array_filter(array_merge(request()->except(['type', 'page']), ['type' => $type]))) }}" class="shrink-0 rounded-full border px-3 py-2 text-[13px] font-medium {{ ($filters['type'] ?? '') === $type ? 'border-[#155EEF] bg-[#EEF4FF] text-[#155EEF]' : 'border-[#D0D5DD] bg-white text-[#475467]' }}">{{ $label }}</a>
                @endforeach
            </div>
        </div>
    </section>
        ...
        <div class="grid gap-8 lg:grid-cols-[250px_minmax(0,1fr)]">
            <aside class="hidden lg:block" aria-label="Property filters">
                <form action="{{ route('properties.index') }}" method="GET" class="sticky top-5 rounded-xl border border-[#E4E7EC] bg-white p-5">
                    <div class="mb-5 flex items-center justify-between"><h2 class="font-semibold text-[#172033]">Filters</h2><a href="{{ route('properties.index') }}" class="text-sm font-medium text-[#155EEF]">Reset</a></div>
            ...
            <div class="min-w-0">
                <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                    <div><p class="text-[13px] text-[#667085]">{{ $properties->total() }} {{ Str::plural('property', $properties->total()) }}</p><h1 class="text-[20px] font-semibold tracking-tight text-[#172033] sm:text-2xl">{{ ($filters['q'] ?? null) ? 'Results for “'.$filters['q'].'”' : 'Properties for rent' }}</h1></div>
                ...
                @else
                    <div :class="viewMode === 'list' ? 'view-list grid-cols-1' : 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3'" class="grid gap-4 lg:gap-5">@foreach($properties as $property)<x-property-card :property="$property" />@endforeach</div>
    ...
    <div x-cloak x-show="filterOpen" @keydown.escape.window="filterOpen = false" class="fixed inset-0 z-[65] lg:hidden" role="dialog" aria-modal="true" aria-labelledby="mobile-filter-title">
        ...
        <form action="{{ route('properties.index') }}" method="GET" x-show="filterOpen" x-transition:enter="transition duration-150" x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0" class="absolute inset-x-0 bottom-0 flex max-h-[92dvh] flex-col rounded-t-2xl bg-white">
            <div class="mx-auto mt-3 h-1 w-10 rounded-full bg-[#D0D5DD]"></div><div class="flex items-center justify-between border-b px-4 py-3"><h2 id="mobile-filter-title" class="text-lg font-semibold">Filters</h2><button type="button" @click="filterOpen = false" class="touch-icon -mr-2" aria-label="Close filters"><x-icon name="x" /></button></div>
            <input type="hidden" name="q" value="{{ $filters['q'] ?? '' }}"><input type="hidden" name="type" value="{{ $filters['type'] ?? '' }}">
