...
<x-layouts.public>
    ...
    <div class="mx-auto max-w-7xl px-4 pb-28 pt-5 sm:px-6 md:pb-14 lg:px-8 lg:pt-7">
        <a href="{{ route('properties.index') }}" class="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-[#475467] hover:text-[#155EEF]"><x-icon name="arrow-left" class="size-4" />Back to listings</a>
        ...
        <div class="mt-7 grid gap-10 lg:grid-cols-[minmax(0,1fr)_340px]">
            <article class="min-w-0">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div><div class="flex flex-wrap items-center gap-2"><x-status-badge :type="$property->availability_status === 'available' ? 'available' : 'reserved'">{{ str($property->availability_status)->headline() }}</x-status-badge><x-status-badge>{{ $property->type_label }}</x-status-badge></div><p class="mt-4 text-3xl font-bold tracking-tight text-[#0B2A5B] md:text-4xl">{{ $property->formatted_rent }}</p><h1 class="mt-2 text-2xl font-bold leading-tight tracking-tight text-[#172033] md:text-3xl">{{ $property->title }}</h1><p class="mt-3 flex items-center gap-2 text-[#667085]"><x-icon name="map-pin" class="size-5 text-[#155EEF]" />{{ $property->display_address }}</p></div>
                ...
                <section class="mt-8 grid grid-cols-2 gap-3 rounded-xl border border-[#E4E7EC] bg-white p-4 sm:grid-cols-4" aria-label="Property facts">
                    @foreach([
                    ...
                    ] as [$icon, $fact]) @if($fact)<div class="flex min-h-16 items-center gap-3 rounded-lg bg-[#F7F9FC] px-3"><x-icon :name="$icon" class="size-5 shrink-0 text-[#155EEF]" /><span class="text-sm font-semibold text-[#344054]">{{ $fact }}</span></div>@endif @endforeach
                </section>

                <section class="mt-9" aria-labelledby="description-title"><h2 id="description-title" class="text-xl font-bold text-[#172033]">About this property</h2><p class="mt-3 whitespace-pre-line text-base leading-8 text-[#475467]">{{ $property->description }}</p><p class="mt-3 text-sm text-[#667085]">Always confirm fees, availability and terms directly before making a commitment.</p></section>

                <section class="mt-9 border-t border-[#E4E7EC] pt-8" aria-labelledby="amenities-title"><h2 id="amenities-title" class="text-xl font-bold text-[#172033]">Amenities</h2><div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">@foreach($property->amenities as $amenity)<div class="flex min-h-12 items-center gap-2.5 rounded-lg bg-white px-3 text-sm font-medium text-[#344054]"><span class="flex size-7 items-center justify-center rounded-full bg-[#EEF4FF] text-[#155EEF]"><x-icon name="check" class="size-4" /></span>{{ $amenity->amenity_label }}</div>@endforeach</div></section>

                <section class="mt-9 rounded-xl border border-[#D7E2F4] bg-white p-5" aria-labelledby="agent-title">
                    <div class="flex items-start gap-4"><div class="flex size-12 shrink-0 items-center justify-center rounded-full bg-[#0B2A5B] text-lg font-bold text-white">{{ Str::of($property->agent->display_name)->explode(' ')->map(fn($part) => Str::substr($part, 0, 1))->take(2)->join('') }}</div><div><div class="flex flex-wrap items-center gap-2"><h2 id="agent-title" class="font-bold text-[#172033]">{{ $property->agent->display_name }}</h2>@if($property->agent->isVerified())<x-status-badge type="verified" icon="shield-check">Verified agent</x-status-badge>@endif</div><p class="mt-1 text-sm text-[#667085]">{{ $property->agent->primary_location }} property professional</p><p class="mt-3 max-w-2xl text-sm leading-6 text-[#475467]">{{ $property->agent->short_bio }}</p></div></div>
                </section>
            </article>

            <aside class="hidden lg:block">
                <div class="sticky top-5 rounded-xl border border-[#D7E2F4] bg-white p-5 shadow-[0_12px_35px_rgba(11,42,91,.08)]">
                    <p class="text-sm font-medium text-[#667085]">Annual rent</p><p class="mt-1 text-2xl font-bold text-[#0B2A5B]">{{ $property->formatted_rent }}</p><p class="mt-2 text-sm leading-5 text-[#667085]">Confirm all charges with the agent before payment.</p>
    ...
    <div class="fixed inset-x-0 bottom-0 z-30 grid grid-cols-2 gap-2 border-t border-[#E4E7EC] bg-white px-3 py-3 pb-[max(.75rem,env(safe-area-inset-bottom))] shadow-[0_-6px_20px_rgba(11,42,91,.08)] lg:hidden">
        ...
        @guest<button type="button" @click="openLogin('tenant')" :disabled="!online" class="btn-secondary px-3 disabled:opacity-60"><x-icon name="chat" class="size-4" />Chat with Agent</button>@else<button type="button" @click="showToast('Secure chat arrives in Sprint 8.')" class="btn-secondary px-3"><x-icon name="chat" class="size-4" />Chat with Agent</button>@endguest
    </div>

    <div x-cloak x-show="infoOpen" @keydown.escape.window="infoOpen = false" class="fixed inset-0 z-[75] flex items-end justify-center p-0 md:items-center md:p-4" role="dialog" aria-modal="true" aria-labelledby="inspection-title"><button @click="infoOpen = false" class="absolute inset-0 bg-[#06152D]/75" aria-label="Close"></button><section x-show="infoOpen" x-transition class="relative w-full rounded-t-2xl bg-white p-6 md:max-w-md md:rounded-xl"><button @click="infoOpen = false" class="touch-icon absolute right-3 top-3" aria-label="Close"><x-icon name="x" /></button><span class="flex size-12 items-center justify-center rounded-lg bg-[#EEF4FF] text-[#155EEF]"><x-icon name="calendar" class="size-6" /></span><h2 id="inspection-title" class="mt-4 text-xl font-bold">Inspection booking is coming in Sprint 5</h2><p class="mt-2 text-sm leading-6 text-[#667085]">You will be able to choose an available time and track the visit here. No booking has been created.</p><button @click="infoOpen = false" class="btn-primary mt-5 w-full">Got it</button></section></div>
</x-layouts.public>
